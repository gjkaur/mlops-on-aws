"""Sklearn realtime inference for Lab 8 churn inputs (seven CSV floats matching preprocess order)."""
...
def input_fn(request_body, request_content_type):
    ...
    if "text/csv" in ct or ct == "":
        vals = np.array([float(x) for x in raw.strip().split(",")], dtype=np.float64)
